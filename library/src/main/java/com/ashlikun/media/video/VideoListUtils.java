...
public class VideoListUtils {
    ...
    public static void onChildViewAttachedToWindow(View view, int vidoPlayId) {
        if (EasyVideoPlayerManager.getVideoTiny() != null) {
            ...
            if (videoPlayer != null && VideoUtils.getCurrentMediaData(videoPlayer.getMediaData(), videoPlayer.getCurrentUrlIndex()).equals(EasyMediaManager.getCurrentDataSource())) {
        ...
        if (EasyVideoPlayerManager.getVideoTiny() == null) {
            ...
            if (videoPlayer instanceof View && ((ViewGroup) view).indexOfChild(videoPlayer) != -1) {
                if (videoPlayer.getCurrentState() == PAUSE) {
                    ...
                    if (VideoScreenUtils.startWindowTiny(new EasyVideoPlayTiny(videoPlayer.getContext()), videoPlayer.getMediaData(), videoPlayer.getCurrentUrlIndex())) {
                        //还原默认状态
            ...
            if ((currentPlayPosition < firstVisibleItem || currentPlayPosition > (lastVisibleItem - 1))) {
                if (EasyVideoPlayerManager.getVideoTiny() == null) {
                    if (EasyVideoPlayerManager.getCurrentVideoPlayerNoTiny().getCurrentState() == PAUSE) {
                        ...
                        BaseEasyVideoPlay videoPlayer = EasyVideoPlayerManager.getCurrentVideoPlayerNoTiny();
                        VideoScreenUtils.startWindowTiny(new EasyVideoPlayTiny(videoPlayer.getContext()), videoPlayer.getMediaData(), videoPlayer.getCurrentUrlIndex());
                        videoPlayer.setStatus(VideoStatus.NORMAL);
                    }
                }
            } else {
    ...
    public static void onRecyclerAutoTiny(EasyVideoPlayer videoPlayer, boolean isChileViewDetached) {
        ...
        if (videoPlayer == null || !VideoUtils.isContainsUri(videoPlayer.getMediaData(),
        ...
        if (isChileViewDetached) {
            if (EasyVideoPlayerManager.getVideoTiny() == null) {
                if (EasyVideoPlayerManager.getCurrentVideoPlayerNoTiny() != null &&
                    ...
                    if (VideoScreenUtils.startWindowTiny(new EasyVideoPlayTiny(videoPlayer.getContext()),
